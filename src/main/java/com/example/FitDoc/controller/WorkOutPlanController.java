...
import com.example.FitDoc.repository.WorkOutPlanRepository;
...
public class WorkOutPlanController {
    ...
    public WorkOutPlanController(UserRepository userRepository) {
        this.userRepository = userRepository;
    }

    @GetMapping()
    public String getAllPosts(@AuthenticationPrincipal OAuth2User user) {
        List<WorkOutPlan> posts = WorkOutPlanRepository.findAll();
        String userId = (String) user.getAttribute("email");

        JsonArray jsonArray = new JsonArray();

        for (WorkOutPlan post : posts) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<WorkOutPlan> getPostById(@PathVariable String id) {
        WorkOutPlan post = WorkOutPlanRepository.findById(id).orElse(null);
        if (post != null) {
    ...
    @PostMapping
    public ResponseEntity<WorkOutPlan> createPost(@RequestBody WorkOutPlan post, @AuthenticationPrincipal OAuth2User user) {
        ...
        WorkOutPlan savedPost = WorkOutPlanRepository.save(post);
        return new ResponseEntity<>(savedPost, HttpStatus.CREATED);
    }

    @GetMapping("/like/{id}")
    public ResponseEntity<WorkOutPlan> likePost(@PathVariable String id, @AuthenticationPrincipal OAuth2User user) {
        return getPostResponseEntity(id, user, WorkOutPlanRepository);
    }

    ResponseEntity<WorkOutPlan> getPostResponseEntity(@PathVariable String id, @AuthenticationPrincipal OAuth2User user, WorkOutPlanRepository WorkOutPlanRepository) {
        WorkOutPlan post = WorkOutPlanRepository.findById(id).orElse(null);
        if (post != null) {
            String userId = (String) user.getAttribute("email");
            ...
            WorkOutPlanRepository.save(post);
    ...
    public ResponseEntity<WorkOutPlan> dislikePost(@PathVariable String id, @AuthenticationPrincipal OAuth2User user) {
        WorkOutPlan post = WorkOutPlanRepository.findById(id).orElse(null);
        if (post != null) {
            String userId = (String) user.getAttribute("email");
    ...
    @PutMapping("/{id}")
    public ResponseEntity<WorkOutPlan> editPost(@PathVariable String id, @RequestBody WorkOutPlan post) {
        WorkOutPlan postToEdit = WorkOutPlanRepository.findById(id).orElse(null);
        if (postToEdit != null) {
            postToEdit.setContent(post.getContent());
            WorkOutPlanRepository.save(postToEdit);
    ...
    public ResponseEntity<Comment> addCommentToPost(@PathVariable String postId, @RequestBody Map<String, String> commentData, @AuthenticationPrincipal OAuth2User user) {
        WorkOutPlan post = WorkOutPlanRepository.findById(postId).orElse(null);
        if (post == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
